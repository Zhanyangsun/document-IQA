...
def create_directories(base_dir, noise_name):
    os.makedirs(base_dir, exist_ok=True)
    os.makedirs(os.path.join(base_dir, noise_name), exist_ok=True)
...
# Function to apply a single distortion to an image and save the results
def apply_and_save_single_distortion(image_paths, base_dir, noise_type):
    distortion_levels = {
        ...
        'pixelate': [i * 0.2 + 1 for i in range(0, 10)],  # Pixelate levels from 1 to 2
        'jitter': list(range(1, 11)),  # Jitter levels from 1 to 10
    ...
    create_directories(base_dir, noise_type)
    noise_dir = os.path.join(base_dir, noise_type)

    for src_path, img_name in image_paths:
        image = Image.open(src_path)

        # Save the reference image with ground truth zero
        ref_image_path = os.path.join(noise_dir, img_name)
        image.save(ref_image_path, quality=100)
        ground_truth.append({'image': img_name, 'distortion_level': [0]})

        # Generate 20 random distortion levels for the noise
        for _ in range(20):
            level = random.choice(distortion_levels[noise_type])

            distorted_image = image.copy()

            if noise_type == 'binary_threshold':
                distorted_image = apply_binary_threshold(distorted_image, level)
            elif noise_type == 'gaussian_blur':
                distorted_image = apply_gaussian_blur(distorted_image, level)
            elif noise_type == 'optical_problems':
                distorted_image = apply_optical_problems(distorted_image, level)
            elif noise_type == 'rotation':
                distorted_image = apply_page_rotation(distorted_image, level)
            elif noise_type == 'contrast_change':
                distorted_image = add_contrast_change_noise(distorted_image, level)
            elif noise_type == 'pixelate':
                distorted_image = add_pixelate_noise(distorted_image, level)
            elif noise_type == 'jitter':
                distorted_image = add_jitter_noise(distorted_image, level)
            elif noise_type == 'mean_shift':
                distorted_image = add_mean_shift_noise(distorted_image, level)
            elif noise_type == 'salt_and_pepper':
                distorted_image = add_salt_and_pepper_noise(distorted_image, level)

            distorted_img_name = f'{os.path.splitext(img_name)[0]}_{level}.png'
            distorted_image_path = os.path.join(noise_dir, distorted_img_name)
            distorted_image.save(distorted_image_path, quality=100)

            ground_truth.append({'image': distorted_img_name, 'distortion_level': [level]})

    # Ensure that JSON is properly written
    try:
        with open(os.path.join(noise_dir, 'ground_truth.json'), 'w') as f:
...
def main():
    ...
    # Step 3: Apply distortions and save the results for each noise type
    noise_types = ['jitter']#['binary_threshold', 'gaussian_blur', 'optical_problems', 'rotation', 'contrast_change', 'pixelate', 'jitter', 'mean_shift', 'salt_and_pepper']

    for noise_type in noise_types:
        apply_and_save_single_distortion(image_paths, base_dir, noise_type)
